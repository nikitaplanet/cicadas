<template>
	<div
		v-if="!detailData"
		class="w-full h-screen text-center pt-32 pb-32 flex justify-center items-center px-6 text-sm text-link-primary md:px-10">
		無此頁資訊
				...
				<div class="w-full grid grid-cols-1 gap-5 lg:grid-cols-2 md:gap-12">
					<div class="flex flex-col justify-start items-start">
						<div
							...
							<img
								alt=""
								src="@/assets/img/icons/sliderDots/dot_active.svg" />
							<span class="ml-2 text-text-primary font-label text-scaleDef italic font-semibold">
								Ongoing
							</span>
						</div>
						<h2
...
const data = studioWording.studioInfo.find(
	(item: StudioData) => item.id === String(route.params.id),
);
